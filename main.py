...
def fetch_instagram_posts():
    ...
    # Session auth check — make a real authenticated API call to confirm the session is valid.
    try:
        verified_username = loader.test_login()
        if not verified_username:
            print(f"WARN: Instagram session auth check failed for {instagram_username} — session not authenticated")
            _notify_health_monitor(
            ...
            return []
        print(f"Instagram session valid: logged in as @{verified_username}")
    except Exception as e:
        print(f"WARN: Instagram session auth check raised an exception for {instagram_username}: {e}")
        _notify_health_monitor(
            f"⚠️ Instagram session auth check error for {instagram_username}: {e}\n"
            f"Re-authenticate and update the INSTAGRAM_SESSION secret."
        )
        return []
