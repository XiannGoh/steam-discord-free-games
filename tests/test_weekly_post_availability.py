...
def _run_main(monkeypatch, tmp_path, *, existing_state, fake_client):
    ...
    monkeypatch.setattr(weekly, "post_or_edit_rolling_explainer", lambda *a, **kw: rolling_calls.append(a))
    ...
    weekly.main()
    return json.loads(path.read_text(encoding="utf-8")), rolling_calls
...
def test_rerun_reuses_intro_and_days_without_duplicates(monkeypatch, tmp_path):
    ...
    saved, _ = _run_main(monkeypatch, tmp_path, existing_state=existing, fake_client=fake)
...
def test_rolling_explainer_called_once_after_posting(monkeypatch, tmp_path):
    """Rolling explainer must be called exactly once at the end of the main flow."""
    fake = FakeClient()

    _, rolling_calls = _run_main(monkeypatch, tmp_path, existing_state={}, fake_client=fake)

    assert len(rolling_calls) == 1
    assert rolling_calls[0][2] == "weekly-scheduling"
